...
import kossh.operations.TransferOperations
import kossh.util.readUntil
import kossh.util.toCharset
import java.io.*
...
open class SSHScp(val ssh: SSH): TransferOperations, Closeable {
    ...
    fun remoteFileToOutputStream(
        ...
        try {
            ...
            val sin = ch.getInputStream().buffered()
            ...
            fun bufAppend(x: Int) { buf.append(x.toChar()) }
            fun bufReset() { buf.setLength(0) }
    ...
    private fun checkAck(input: InputStream): Int {
        ...
        return when(readVal) {
            1 -> throw RuntimeException("kossh.impl.SSH transfert protocol error " + consumeMessage())
            2 -> throw RuntimeException("kossh.impl.SSH transfert protocol fatal error " + consumeMessage())
            else -> readVal
        }
    }

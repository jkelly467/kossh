...
open class SSH(val options: SSHOptions) : AllOperations, Closeable {
    ...
    fun jschsession(): Session {
        ...
        try {
            ...
            try {checkedsession.disconnect()} catch (e: Exception) {}
        ...
        options.identities.forEach { ident ->
            ...
            if (fident.isFile) {
                ...
                if (pass != null) {
                    ...
                    jsch.addIdentity(fident.absolutePath)
                }
            }
        }
    ...
    private fun<T> opWithFallback(primary: ()->T, fallback: ()->T): T {
        if (firstHasFailed) {
            ...
            return try {
                ...
                if (e.message.orEmpty().contains("kossh.impl.SSH transfert protocol error")) {
                    firstHasFailed = true
                }
                fallback()
            }
